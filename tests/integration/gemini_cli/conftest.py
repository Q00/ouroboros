...
Usage:
    def test_adapter_happy_path(gemini_subprocess_stub, gemini_session_id):
        gemini_subprocess_stub.queue(
            ...
            stdout_events=[
                gemini_event_init(gemini_session_id),
...
def gemini_event_init(session_id: str) -> dict[str, Any]:
    """Create a ``init`` JSONL event.
...
def gemini_event_tool_use(
    name: str,
    args: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Create a ``tool_use`` JSONL event.
...
def gemini_event_result(*, exit_code: int = 0) -> dict[str, Any]:
...
class FakeGeminiStream:
    ...
    Example::
        ...
        stream = FakeGeminiStream('{"type": "result", "exit_code": 0}\\n')
        data = await stream.read(16384)
        assert data == b'{"type": "result", "exit_code": 0}\\n'
    """
...
class FakeGeminiProcess:
    ...
    Example::
        ...
        process = FakeGeminiProcess(
            stdout_text='{"type": "result", "exit_code": 0}\\n',
            returncode=0,
            stdin=FakeGeminiStdin(),
        )
...
class FakeGeminiEventStreamEmitter:
    ...
    Example::
        ...
        emitter = FakeGeminiEventStreamEmitter([
            gemini_event_init("sess-abc"),
            gemini_event_thinking("Let me read the files first."),
            gemini_event_tool_use("Read", {"file_path": "src/foo.py"}),
...
def make_happy_path_events(
    ...
    Args:
        session_id: Session ID to include in the ``init`` event.
...
def make_tool_use_events(
    ...
    Args:
        session_id: Session ID for the ``init`` event.
    ...
    return [
        ...
        gemini_event_tool_use(tool_name, tool_args or {"file_path": "src/foo.py"}),
...
def gemini_happy_path_events(gemini_session_id: str) -> list[dict[str, Any]]:
    ...
    Returns:
        List of event dicts: init → thinking → message → done.
    """
...
def gemini_tool_use_events(gemini_session_id: str) -> list[dict[str, Any]]:
    ...
    Returns:
        List of event dicts: init → thinking → tool_use
...
def gemini_error_events(gemini_session_id: str) -> list[dict[str, Any]]:
    ...
    Returns:
        List of event dicts: init → error.
    """
