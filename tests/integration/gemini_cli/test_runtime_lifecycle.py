...
class TestGeminiCLIRuntimeProcessStart:
    ...
    @pytest.mark.asyncio
    async def test_no_stdin_pipe_requested(self) -> None:
        """The subprocess must be started without an interactive stdin pipe."""
        captured_kwargs: dict[str, Any] = {}
    ...
    @pytest.mark.asyncio
    async def test_prompt_passed_as_argument(self) -> None:
        """The task prompt must be passed via the --prompt CLI argument."""
        captured_args: list[str] = []

        async def _fake_exec(*args: str, **_kwargs: Any) -> _FakeProcess:
            captured_args.extend(args)
            return _FakeProcess(stdout_text="response\n", returncode=0)
        ...
        assert captured_args[idx + 1] == "Write unit tests"
...
class TestGeminiCLIRuntimeErrorHandling:
    ...
    async def test_json_event_with_session_id_builds_runtime_handle(self) -> None:
        ...
        session_event = json.dumps({"type": "init", "session_id": "sess-xyz"})
        ...
        assert any(h.native_session_id == "sess-xyz" for h in handles), (
    ...
    async def test_only_whitespace_stdout_lines_are_filtered(self) -> None:
        ...
        for msg in assistant_messages:
            assert msg.content.strip() != "", f"Blank assistant message emitted: {msg!r}"
    ...
    @pytest.mark.asyncio
    async def test_runtime_factory_creates_gemini_cli_runtime(self, tmp_path: Path) -> None:
