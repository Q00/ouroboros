...
_EXEC_PATCH_TARGET = "ouroboros.orchestrator.codex_cli_runtime.asyncio.create_subprocess_exec"
...
def _build_evaluation_context(
    ...
    *,
    ...
    """
    artifact_parts = [
        msg.content for msg in messages if msg.type in {"assistant", "result"} and msg.content
    ]
...
class TestGeminiCLIRuntimeE2ERun:
    ...
    async def test_runtime_captures_hello_world_output(self) -> None:
        ...
        assert expected_output in all_content, (
            f"Expected '{expected_output}' in collected message content, got: {all_content!r}"
        )
    ...
    async def test_evaluation_pipeline_approves_gemini_output(self) -> None:
        ...
        assert eval_result.is_ok, f"EvaluationPipeline returned an error: {eval_result.error}"
        result: EvaluationResult = eval_result.value
        assert result.final_approved is True, (
            f"Expected final_approved=True but got False. Stage 2: {result.stage2_result}"
        )
        assert result.stage2_result is not None, "Stage 2 semantic result must be populated."
    ...
    async def test_full_pipeline_message_count(self) -> None:
        ...
        for line, content in zip(stdout_lines, assistant_contents, strict=False):
            assert content == line, f"Expected message content {line!r}, got {content!r}"
...
class TestGeminiCLIRuntimeE2EEdge:
    ...
    async def test_evaluation_context_artifact_contains_runtime_output(self) -> None:
        ...
        assert expected_text in context.artifact, (
            f"Expected runtime output in evaluation context artifact. artifact={context.artifact!r}"
        )
    ...
    async def test_gemini_cli_backend_alias_via_factory(self, tmp_path: Path) -> None:
        ...
        assert isinstance(runtime, GeminiCLIRuntime), (
            f"Expected GeminiCLIRuntime for 'gemini_cli' backend, got {type(runtime).__name__}"
        )
