...
def test_built_wheel_preserves_packaging_contracts(tmp_path: Path) -> None:
    ...
    for profiles in ("mcp,claude", "mcp,claude-sdk", "all,mcp"):
        ...
        # The wheel path fixes the candidate to this exact build, so the
        # resolver cannot silently backtrack to an older Ouroboros release.
        # Both conflicting pins and both requested profiles must survive in
        # the diagnostic, proving this is the intentional MCP boundary.
        assert "mcp==1.28.1" in resolver_error
        assert "mcp==2.0.0" in resolver_error
        for profile in profiles.split(","):
            assert f"ouroboros-ai[{profile}]" in resolver_error
