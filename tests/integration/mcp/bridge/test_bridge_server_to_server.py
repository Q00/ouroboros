...
class TestMCPBridgeReusability:
    def test_bridge_config_accepts_multiple_servers(self):
        config = MCPBridgeConfig(
            servers=(
                MCPServerConfig(name="calc", transport=TransportType.STDIO, command="echo", args=("1",)),
                MCPServerConfig(name="greet", transport=TransportType.STDIO, command="echo", args=("2",)),
            ),
    ...
    def test_bridge_creates_manager(self):
        config = MCPBridgeConfig(
            servers=(MCPServerConfig(name="a", transport=TransportType.STDIO, command="echo", args=()),),
        )
    ...
    async def test_lifecycle_with_multiple_configs(self):
        config = MCPBridgeConfig(
            servers=(
                MCPServerConfig(name="s1", transport=TransportType.STDIO, command="echo", args=("1",)),
                MCPServerConfig(name="s2", transport=TransportType.STDIO, command="echo", args=("2",)),
            ),
        )
        bridge = MCPBridge.from_config(config)
        with patch.object(bridge._manager, "add_server", new_callable=AsyncMock) as ma, \
             patch.object(bridge._manager, "connect_all", new_callable=AsyncMock, return_value={"s1": Result.ok(None), "s2": Result.ok(None)}):
