...
class TestGeminiCLIAdapterInterviewStage:
    ...
    @pytest.mark.asyncio
    async def test_adapter_records_response_and_advances_round(self, tmp_path: Path) -> None:
...
class TestGeminiCLIFullInterviewSeedPipeline:
    ...
    async def test_full_pipeline_completes_successfully(self, tmp_path: Path) -> None:
        ...
        assert len(seed.acceptance_criteria) >= 1, (
            "Seed must have at least one acceptance criterion"
        )
...
class TestGeminiCLIRuntimePipelineWiring:
    ...
    def test_cli_start_accepts_llm_backend_gemini(self) -> None:
        ...
        assert result.exit_code == 0, f"CLI rejected --llm-backend gemini. Output:\n{result.output}"
    ...
    def test_cli_start_accepts_runtime_gemini(self) -> None:
        ...
        assert result.exit_code == 0, f"CLI rejected --runtime gemini. Output:\n{result.output}"

    def test_mcp_serve_accepts_llm_backend_gemini(self) -> None:
        """The mcp serve CLI command accepts --llm-backend gemini without error."""
        from unittest.mock import AsyncMock, patch
        ...
        with (
            patch("ouroboros.cli.commands.mcp._run_mcp_server", new_callable=AsyncMock),
            patch("ouroboros.cli.commands.mcp._check_stale_instance", return_value=False),
            patch("ouroboros.cli.commands.mcp._write_pid_file", return_value=True),
            patch("ouroboros.cli.commands.mcp._cleanup_pid_file", return_value=None),
        ):
            # Pass transport stdio to avoid hanging server
            result = cli_runner.invoke(
                app,
                [
                    "mcp",
    ...
    def test_mcp_info_accepts_llm_backend_gemini(self) -> None:
        """The mcp info CLI command accepts --llm-backend gemini without error."""
        ...
        with patch(
            "ouroboros.cli.commands.mcp.create_ouroboros_server", create=True
        ) as mock_create:
            mock_server = MagicMock()
            mock_server.info.name = "ouroboros-mcp"
            mock_server.info.version = "1.0.0"
            mock_server.info.capabilities.tools = True
            mock_server.info.capabilities.resources = False
            mock_server.info.capabilities.prompts = False
