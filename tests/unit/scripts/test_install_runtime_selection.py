...
def test_explicit_claude_isolates_mcp_from_claude_extra(tmp_path: Path) -> None:
    ...
    assert (
        "uv tool install --upgrade --python >=3.12 . --with click>=8.1.0,<9.0.0 --with claude-agent-sdk==0.2.128 --with anthropic==0.120.0 --with textual==8.2.8 --with textual-serve==1.1.3"
        in calls
    )
