...
class TestTimeoutBehavior:
    ...
    async def test_connect_to_nonexistent_command_does_not_hang(self):
        config = MCPBridgeConfig(
            servers=(MCPServerConfig(name="ghost", transport=TransportType.STDIO, command="nonexistent_command_xyz_12345", args=()),),
            timeout_seconds=3.0, retry_attempts=1,
        )
    ...
    async def test_partial_failure_does_not_hang(self):
        config = MCPBridgeConfig(
            servers=(
                MCPServerConfig(name="ok", transport=TransportType.STDIO, command="echo", args=("1",)),
                MCPServerConfig(name="bad", transport=TransportType.STDIO, command="false", args=()),
            ),
            timeout_seconds=3.0, retry_attempts=1,
        )
        bridge = MCPBridge.from_config(config)
        ...
        with patch.object(bridge._manager, "add_server", new_callable=AsyncMock), \
             patch.object(bridge._manager, "connect_all", new_callable=AsyncMock, return_value=mock_results):
            results = await bridge.connect()
...
class TestConnectionLifecycle:
    ...
    async def test_close_after_failed_connect(self):
        config = MCPBridgeConfig(servers=(MCPServerConfig(name="fail", transport=TransportType.STDIO, command="false", args=()),))
        bridge = MCPBridge.from_config(config)
        with patch.object(bridge._manager, "add_server", new_callable=AsyncMock), \
             patch.object(bridge._manager, "connect_all", new_callable=AsyncMock, side_effect=Exception("boom")):
...
class TestResourceCleanup:
    ...
    async def test_context_manager_cleans_up_on_exception(self):
        ...
        with patch.object(bridge, "connect", new_callable=AsyncMock), \
