...
class TestExecuteSeedHandler:
    ...
    def test_start_execute_seed_handler_factory_accepts_runtime_and_llm_backends(self) -> None:
        """Async wrapper helper should preserve runtime selection through its nested execute handler."""
        handler = start_execute_seed_handler(runtime_backend="codex", llm_backend="litellm")
        assert handler._execute_handler.agent_runtime_backend == "codex"
        assert handler._execute_handler.llm_backend == "litellm"
...
class TestEvaluateHandler:
    ...
    async def test_handle_success(self) -> None:
        ...
        mock_stage2 = SemanticResult(
            ...
            questions_used=("What evidence shows the artifact satisfies the AC?",),
            evidence=("The artifact includes the expected return value.",),
        )
        ...
        assert "Questions Used:" in result.value.text_content
        assert "Evidence:" in result.value.text_content
...
class TestEvaluateHandlerCodeChanges:
    ...
    def test_format_result_includes_questions_and_evidence(self) -> None:
        """Stage 2 formatting exposes evaluator questions and evidence."""
        ...
        result = EvaluationResult(
            ...
            stage2_result=SemanticResult(
                ...
                drift_score=0.08,
                uncertainty=0.1,
                reasoning="Evaluator checked the implementation against the stated goal.",
                questions_used=("What evidence shows the output addresses the root problem?",),
                evidence=("The implementation includes the expected verification path.",),
            ),
            final_approved=True,
        )

        text = handler._format_evaluation_result(result)

        assert "Questions Used:" in text
        assert "Evidence:" in text
        assert "What evidence shows the output addresses the root problem?" in text
        assert "The implementation includes the expected verification path." in text
