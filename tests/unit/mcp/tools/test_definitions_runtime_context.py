...
class TestGetOuroborosTools:
    ...
    def test_context_bridge_propagates_to_auto_handler(self) -> None:
        manager = object()
        bridge = _FakeBridge(manager=manager, tool_prefix="ctx_")
        ...
        auto_handler = next(h for h in handlers if h.definition.name == "ouroboros_auto")
        assert auto_handler.mcp_manager is manager
        assert auto_handler.mcp_tool_prefix == "ctx_"
