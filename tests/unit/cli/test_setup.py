...
class TestGjcSetup:
    ...
    def test_detect_runtimes_includes_gjc_from_path(self) -> None:
        ...
        with (
            patch("ouroboros.cli.commands.setup.shutil.which", side_effect=lambda name: which_calls.get(name)),
            patch("ouroboros.config.get_gemini_cli_path", return_value=None),
    ...
    def test_setup_gjc_writes_config_artifact_and_bridge(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        config_dir = tmp_path / ".ouroboros"
        config_dir.mkdir()
        (config_dir / "config.yaml").write_text("{}", encoding="utf-8")
        ...
        assert guide_path.read_text(encoding="utf-8") == render_backend_skill_capability_guide("gjc")

        bridge_path = agent_dir / "extensions" / "ouroboros-ooo-bridge" / "index.ts"
        bridge = bridge_path.read_text(encoding="utf-8")
        assert not (agent_dir / "extensions" / "ouroboros-ooo-bridge" / "ouroboros-ooo-bridge.ts").exists()
