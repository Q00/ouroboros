...
class TestManifestContract:
    ...
    def test_v03_manifest_accepts_observability_hook(
    ...
    @pytest.mark.parametrize("hook_name", sorted(TERMINAL_OBSERVABILITY_HOOK_NAMES))
    def test_fail_closed_observability_hook_rejected(
        self, tmp_path: Path, hook_name: str
    ) -> None:
        payload = _payload_with_hooks(hook_name)
...
class TestOnErrorDispatch:
    ...
    def test_nonzero_exit_dispatches_on_error_after_terminal_failed(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_no_on_error_hook_declared_does_not_emit_hook_events(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_on_error_hook_failure_does_not_mask_original_error(
        self, tmp_path: Path
    ) -> None:
        ...
        result = invoke_plugin(
            ...
            subprocess_runner=_runner_factory(
                hook_returncode=7, command_returncode=42
            ),
        )
        ...
        hook_failed = [
            event for event in events if event["event_type"] == HOOK_FAILED_EVENT
        ]
...
class TestOnCancelDispatch:
    ...
    def test_on_cancel_hook_failure_does_not_mask_cancellation(
        self, tmp_path: Path
    ) -> None:
