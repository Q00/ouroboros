...
def _install_linux_procfs(
    ...
    *,
    ...
    def fake_read_bytes(path: Path, *args, **kwargs):
        ...
        if path == heartbeat._LINUX_BOOT_ID_PATH:
            return boot_id + b"\n"
...
def test_linux_identity_is_stable_while_btime_drifts(
    ...
    for _ in range(10):
        assert (
            heartbeat.persisted_process_identity_alive(
                identity,
                legacy_pid=pid,
...
def test_linux_identity_rejects_dead_process(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(heartbeat.os, "kill", missing)
...
def test_non_linux_behavior_delegates_to_legacy_identity(
    ...
    monkeypatch.setattr(heartbeat, "is_process_identity_alive", legacy_alive)

    assert heartbeat.current_persisted_process_identity() is None
    assert (
        heartbeat.persisted_process_identity_alive(
            None,
            legacy_pid=77,
