...
_EXPECTED_OUROBOROS_TOOL_RETRY = {
    ...
    "ouroboros_ralph": {"supported": True, "mode": "handler_owned"},
...
_EXPECTED_OUROBOROS_TOOL_INTERRUPT = {
    ...
    "ouroboros_cancel_execution": _TERMINAL_CONTROL_INTERRUPTS[
        "ouroboros_cancel_execution"
    ],
    ...
    "ouroboros_ralph": _blocking_interrupt("ouroboros_start_ralph"),
...
_EXPECTED_OUROBOROS_TOOL_CANCEL = {
    ...
    "ouroboros_ralph": _UNSUPPORTED_CANCEL,
...
def test_each_ouroboros_owned_tool_metadata_is_tool_specific_not_generic_inference(
    ...
    def fail_on_generic_attached_metadata(tool: MCPToolDefinition):
        raise AssertionError(
            f"Ouroboros-owned tool used generic attached metadata: {tool.name}"
        )

    def fail_on_generic_attached_semantics(tool: MCPToolDefinition):
        raise AssertionError(
            f"Ouroboros-owned tool used generic attached semantics: {tool.name}"
        )
    ...
    graph = build_capability_graph(
        assemble_session_tool_catalog(attached_tools=(definition,))
    )
    descriptor = graph.by_name()[definition.name]
    ...
    assert metadata.execution_mode == (
        _EXPECTED_OUROBOROS_TOOL_EXECUTION_MODES[definition.name]
    )
    assert metadata.companions == _EXPECTED_OUROBOROS_TOOL_COMPANIONS[
        definition.name
    ]
    assert metadata.required_context_keys == (
        _EXPECTED_OUROBOROS_REQUIRED_CONTEXT_KEYS[definition.name]
    )
    assert metadata.mutation_targets == _EXPECTED_OUROBOROS_TOOL_MUTATION_TARGETS[
        definition.name
    ]
    assert metadata.state_mutations == _EXPECTED_OUROBOROS_TOOL_STATE_MUTATIONS[
        definition.name
    ]
    assert metadata.side_effects == _EXPECTED_OUROBOROS_TOOL_SIDE_EFFECTS[
        definition.name
    ]
...
def test_skill_capability_resolution_maps_core_skills_to_owned_mcp_tools() -> None:
    ...
    assert {skill: descriptor.name for skill, descriptor in resolved.items() if descriptor} == expected
...
def test_core_skills_resolve_end_to_end_to_explicit_mcp_capabilities() -> None:
    ...
    for skill_name, expected_metadata in expected.items():
        ...
        assert descriptor.metadata is not None
        assert descriptor.metadata.fallback_used is False
        assert descriptor.metadata.input_schema == definitions[
            expected_metadata["tool_name"]
        ].to_input_schema()
        ...
        assert descriptor.metadata.state_mutations == (
            _EXPECTED_OUROBOROS_TOOL_STATE_MUTATIONS[
                expected_metadata["tool_name"]
            ]
        )
        ...
        assert descriptor.metadata == ouroboros_tool_capability_registry()[
            expected_metadata["tool_name"]
        ]
        assert resolve_mcp_tool_capability_descriptor(
...
def test_ouroboros_owned_tools_have_explicit_capability_metadata(monkeypatch) -> None:
    ...
    def fail_on_generic_fallback(tool: MCPToolDefinition):
        ...
        raise AssertionError(
            f"Ouroboros-owned tool used generic attached inference: {tool.name}"
        )

    def fail_on_required_parameter_inference(tool: MCPToolDefinition) -> tuple[str, ...]:
        raise AssertionError(
            f"Ouroboros-owned tool used required-parameter inference: {tool.name}"
        )
    ...
    descriptors = {descriptor.name: descriptor for descriptor in graph.capabilities}
    direct_descriptors = {
        descriptor.name: descriptor for descriptor in direct_graph.capabilities
    }
    ...
    for definition in tool_definitions:
        ...
        assert isinstance(metadata.companions, tuple)
        assert all(
            companion in descriptors for companion in metadata.companions
        ), definition.name
        assert metadata.required_context_keys == (
            _EXPECTED_OUROBOROS_REQUIRED_CONTEXT_KEYS[definition.name]
        )
        assert isinstance(metadata.required_context_keys, tuple)
        assert metadata.mutation_targets == (
            _EXPECTED_OUROBOROS_TOOL_MUTATION_TARGETS[definition.name]
        )
        assert metadata.state_mutations == (
            _EXPECTED_OUROBOROS_TOOL_STATE_MUTATIONS[definition.name]
        )
...
def test_ouroboros_capability_registry_covers_every_owned_tool_identifier() -> None:
    ...
    for name, metadata in registry.items():
        ...
        assert all(
            key in metadata.input_schema["properties"] for key in expected_context_keys
        )
...
def test_explicit_ouroboros_tool_capability_lookup_rejects_unknown_ids() -> None:
    ...
    assert (
        lookup_ouroboros_tool_capability_metadata(
            "mcp:external:ouroboros_interview"
        )
        is None
    )
...
def test_ouroboros_owned_tools_have_explicit_valid_execution_mode_assignments() -> None:
    ...
    registry = ouroboros_tool_capability_registry()
    graph = build_capability_graph(assemble_session_tool_catalog(attached_tools=definitions.values()))
    descriptors = {descriptor.name: descriptor for descriptor in graph.capabilities}
...
def test_background_tools_expose_explicit_lifecycle_companion_roles() -> None:
    ...
    background_tools = {
        ...
        "ouroboros_start_ralph": "ouroboros_ralph",
    }
    lifecycle_roles = {
    ...
    for start_tool, blocking_tool in background_tools.items():
        ...
        assert set(lifecycle_roles.values()).issubset(
            descriptor.metadata.companions
        ), start_tool
...
def test_generic_job_lifecycle_tools_derive_sibling_companions_from_catalog(
    ...
    for role, tool_name in lifecycle_roles.items():
        expected_siblings = tuple(
            sibling_name
            for sibling_name in lifecycle_roles.values()
...
def test_generic_job_lifecycle_sibling_metadata_filters_each_absent_sibling(
    ...
    for role, tool_name in available_roles.items():
        ...
        expected_siblings = tuple(
            sibling_name
            for sibling_name in available_roles.values()
            if sibling_name != tool_name
        )
...
def test_background_lifecycle_omits_absent_companion_tools(monkeypatch) -> None:
    ...
    assert "cancel" not in lifecycle["runtime_instruction"]
    assert lifecycle["cancel"] == {
        "supported": False,
        "mode": "unsupported",
        "companions": (),
...
def test_ralph_family_exposes_start_and_lifecycle_companion_metadata() -> None:
    ...
    assert ralph.metadata.execution_mode == "blocking"
    assert set(expected_roles.values()) - {"ouroboros_ralph"} <= set(
        ralph.metadata.companions
    )
    ...
    assert ralph_family["execution_mode"] == "blocking"
    ...
    assert ralph_family["cancel"] == _UNSUPPORTED_CANCEL
    assert ralph_family["background_cancel"] == _BACKGROUND_JOB_CANCEL
    assert "start variant" in ralph_family["runtime_instruction"]
...
def test_ralph_family_omits_absent_lifecycle_companion_tools(monkeypatch) -> None:
    ...
    assert "cancel" not in ralph_family["runtime_instruction"]
    assert "status" in ralph_family["runtime_instruction"]
...
def test_execution_evolution_tools_represent_concrete_execution_side_effects() -> None:
    ...
    for tool_name, expected_state_target in execution_evolution_tools.items():
        ...
        assert {"workspace_write", "event_store_write"} <= set(
            registry_metadata.side_effects
        ), tool_name
        ...
        represented_targets = {
            mutation["target"] for mutation in registry_metadata.state_mutations
        }
...
def test_cancel_tools_represent_runtime_and_persistent_side_effects() -> None:
    ...
    for tool_name, expected in expected_cancel_tools.items():
        ...
        assert {
            mutation["operation"] for mutation in metadata.state_mutations
        } == expected["operations"], tool_name
        assert {
            mutation["side_effect"] for mutation in metadata.state_mutations
        } == expected["side_effects"], tool_name
...
def test_blocking_ouroboros_tools_have_synchronous_interrupt_metadata() -> None:
    ...
    blocking_tools = {
        name
        for name, metadata in registry.items()
        if metadata.execution_mode == "blocking"
    }
    ...
    for name in sorted(blocking_tools):
        ...
        assert all(
            companion in metadata.companions
            for companion in interrupt["background_companions"]
        ), name
...
def test_background_starting_tools_have_resumable_cancellable_interrupt_metadata() -> None:
    ...
    for name in sorted(background_tools):
        ...
        assert descriptor.metadata.interrupt["cancel_companions"] == (
            "ouroboros_cancel_job",
        ), name
...
def test_cancellation_control_tools_have_terminal_control_interrupt_metadata() -> None:
    ...
    for name in sorted(control_tools):
        ...
        assert descriptor.semantics.mutation_class is (
        ...
        assert metadata.interrupt["directive_semantics"].startswith(
            "request_terminal_"
        ), name
...
def test_ouroboros_owned_tools_have_explicit_tool_specific_cancel_behavior() -> None:
    ...
    for name, expected_cancel in _EXPECTED_OUROBOROS_TOOL_CANCEL.items():
        ...
        assert all(
            companion in definitions for companion in metadata.cancel["companions"]
        ), name
...
def test_ouroboros_owned_cancel_lookup_never_uses_generic_attached_inference(
    ...
    def fail_on_generic_fallback(tool: MCPToolDefinition):
        raise AssertionError(
            "Ouroboros-owned cancel lookup used generic attached inference: "
            f"{tool.name}"
        )
...
def test_resolved_ouroboros_mcp_tool_identifier_returns_registered_descriptor() -> None:
    ...
    catalog = assemble_session_tool_catalog(
        attached_tools=(definitions["ouroboros_interview"],)
    )
    graph = build_capability_graph(catalog)
    descriptor = graph.capabilities[0]
...
def test_resolving_ouroboros_mcp_identifiers_never_uses_generic_attached_inference(
    ...
    def fail_on_generic_fallback(tool: MCPToolDefinition):
        raise AssertionError(
            f"Ouroboros-owned lookup used generic attached inference: {tool.name}"
        )
    ...
    implicit_graph_resolved = resolve_mcp_tool_capability_descriptor(
        "ouroboros_interview"
    )
...
def test_interview_capability_exposes_code_investigation_request_model_schema() -> None:
    ...
    assert capability_schema["properties"]["tool_name"] == {
        "const": "ouroboros_interview"
    }
    ...
    assert {
        option["properties"]["target_type"]["const"]
        for option in targets["items"]["oneOf"]
    ...
    assert repo_tool_schema["items"]["properties"]["execution_mode"] == {
        "const": "repo_inspection"
    }
    ...
    assert "[from-code][auto-confirmed]" in schema["properties"]["answer_prefixes"][
        "items"
    ]["enum"]
    ...
    assert "skills/interview/SKILL.md inspect_code PATH 1" in code_investigation[
        "derivation_sources"
    ]
...
def test_code_investigation_subagent_envelope_permits_modeled_repo_inspection_tools() -> None:
    code_investigation = _interview_code_investigation_metadata()
    tool_capabilities = code_investigation["repo_inspection_tool_capabilities"]
    modeled_tool_names = tuple(
        str(capability["tool_name"]) for capability in tool_capabilities
    )
    catalog = assemble_session_tool_catalog(
        builtin_tools=(*modeled_tool_names, "Bash", "Edit")
    )
    ...
    assert set(code_investigation["request_model_schema"]["properties"][
        "allowed_capabilities"
    ]["items"]["enum"]) == {"inspect_code"}
...
def test_code_investigation_owned_mcp_tools_have_explicit_tool_specific_metadata(
    ...
    graph = build_capability_graph(
        assemble_session_tool_catalog(
            attached_tools=(
                definitions[tool_name] for tool_name in code_investigation_mcp_tools
            )
        )
    )
    ...
    for tool_name in code_investigation_mcp_tools:
        ...
        assert metadata.required_context_keys == (
            _EXPECTED_OUROBOROS_REQUIRED_CONTEXT_KEYS[tool_name]
        )
        ...
        assert code_investigation["request_model_schema"]["properties"][
            "mcp_tool_capability"
        ...
        assert set(
            code_investigation["request_model_schema"]["properties"][
                "mcp_tool_capability"
            ]["required"]
...
def test_subagent_job_status_polling_tool_is_explicit_and_callable_in_orchestration_envelope(
    ...
    def fail_on_generic_attached_metadata(tool: MCPToolDefinition):
        raise AssertionError(
            f"subagent status poller used generic attached metadata: {tool.name}"
        )

    def fail_on_generic_attached_semantics(tool: MCPToolDefinition):
        raise AssertionError(
            f"subagent status poller used generic attached semantics: {tool.name}"
        )
...
def test_subagent_job_wait_progress_polling_tool_is_explicit_and_callable_in_orchestration_envelope(
    ...
    assert "linked session, lineage, and subagent events" in (
        metadata.input_schema["properties"]["stream"]["description"]
    )
...
def test_subagent_job_cancellation_tool_is_explicit_and_callable_in_orchestration_envelope(
    ...
    assert cancel_descriptor.semantics.parallel_safety is (
        CapabilityParallelSafety.SERIALIZED
    )
    assert cancel_descriptor.semantics.interruptibility is (
        CapabilityInterruptibility.HARD
    )
    assert cancel_descriptor.semantics.approval_class is (
        CapabilityApprovalClass.ELEVATED
    )
    ...
    assert "status/wait/result/cancel sibling family" in (
        job_lifecycle["runtime_instruction"]
    )
...
def test_interview_capability_exposes_single_code_investigation_answer_contract() -> None:
    ...
    assert "exactly one structured answer payload" in answer_contract[
        "runtime_instruction"
    ]
...
def test_code_investigation_answer_contract_rejects_implicit_or_mismatched_answer() -> None:
    ...
    assert any(
        ...
        and "server_repository_path" in message
...
def test_code_investigation_question_identity_is_stable_from_question_text() -> None:
    ...
    second = stable_code_investigation_question_identity(
        "What framework does this project use?"
    )
    changed = stable_code_investigation_question_identity(
        "What auth method does this project use?"
    )
...
def test_code_investigation_request_model_rejects_malformed_question_identity() -> None:
    ...
    interview = descriptors["ouroboros_interview"]
    assert interview.metadata is not None
    schema = interview.metadata.orchestration["code_investigation"][
    ...
    assert any(
        "'question-1' does not match" in error.message
        and "interview-question" in error.message
        for error in errors
    )
...
def test_code_investigation_request_deserializes_embedded_tool_metadata_without_repo_access(
    ...
    assert capability["required_context_keys"] == expected_capability[
        "required_context_keys"
    ]
...
def test_lateral_persona_panel_metadata_is_structured_without_prose_parsing() -> None:
    ...
    for persona in persona_by_id.values():
        ...
        assert "ouroboros-lateral-inline-dispatch-v1.payloads[persona_id]" in refs[
            "inline_content"
        ]
...
async def test_owned_lateral_thinking_capability_invokes_subagent_orchestration() -> None:
    ...
    assert {entry["execution_mode"] for entry in entries} == {
        "parallel_subagent_panel"
    }
...
async def test_owned_lateral_review_tool_path_uses_explicit_metadata_and_is_callable(
    ...
    review_handler = next(
        ...
        if handler.definition.name == "ouroboros_lateral_think"
    )
    ...
    graph = build_capability_graph(
        assemble_session_tool_catalog(
            attached_tools=(definitions["ouroboros_lateral_think"],)
        )
    )
    descriptor = graph.by_name()["ouroboros_lateral_think"]
    metadata = descriptor.metadata
    ...
    assert {entry["mcp_tool"] for entry in parallel_entries} == {
        "ouroboros_lateral_think"
    }
    assert {entry["execution_mode"] for entry in parallel_entries} == {
        "parallel_subagent_panel"
    }
...
def test_code_investigation_request_model_validates_supported_target_forms() -> None:
    ...
    base_request = _code_investigation_base_request(
        "What framework does this project use?"
    )
    supported_targets = (
...
def test_code_investigation_request_model_requires_explicit_targets() -> None:
    ...
    errors = sorted(
        validator.iter_errors(
            {
                **_code_investigation_base_request(
                    "What framework does this project use?"
                ),
    ...
    assert any("'investigation_targets' is a required property" in error.message for error in errors)
...
def test_code_investigation_request_model_rejects_server_local_repository_fields() -> None:
    ...
    for field_name in (
        ...
        errors = list(
            validator.iter_errors({**base_request, field_name: "/server/local/repo"})
        )
        assert any(
            "Additional properties are not allowed" in error.message
            and field_name in error.message
            for error in errors
        ), field_name
...
def test_owned_mcp_tool_metadata_serialization_round_trips_mutation_fields() -> None:
    ...
    assert serialized[0]["metadata"]["state_mutations"] == [
        ...
        for mutation in _EXPECTED_OUROBOROS_TOOL_STATE_MUTATIONS[
            "ouroboros_interview"
        ]
    ]
    assert serialized[0]["metadata"]["side_effects"] == [
