...
class TestOrdinaryRetryBudgetSurvivesCrashRestart:
    ...
    async def test_finalized_attempt_after_success_fails_closed(
        self,
        later_success: bool,
        later_outcome: str,
    ) -> None:
        """A finalized success is terminal for its root AC."""
        from ouroboros.events.base import BaseEvent

        def _marker(attempt: int, *, success: bool, outcome: str) -> BaseEvent:
            return BaseEvent(
                type="execution.ac.outcome_finalized",
                aggregate_type="execution",
                aggregate_id="exec-original",
        ...
        event_store = AsyncMock()
        event_store.replay.return_value = [
            _marker(0, success=True, outcome="succeeded"),
            _marker(1, success=later_success, outcome=later_outcome),
        ]
        executor = ParallelACExecutor(
            adapter=MagicMock(),
            event_store=event_store,
