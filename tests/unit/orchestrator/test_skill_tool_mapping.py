...
def test_packaged_skill_frontmatter_exposes_declared_mcp_tools() -> None:
    ...
    assert {
        skill: by_skill[skill].skill_path for skill in _CORE_FRONTMATTER_SKILL_TOOLS
    } == {
        skill: f"skills/{skill}/SKILL.md"
        for skill in _CORE_FRONTMATTER_SKILL_TOOLS
    }
...
def test_packaged_skill_frontmatter_exposes_tool_specific_context_keys() -> None:
    ...
    assert skill_frontmatter_context_keys_by_tool(mappings) == {
        by_skill[skill].mcp_tool: context_keys
        for skill, context_keys in expected.items()
    }
...
def test_packaged_skill_bodies_expose_tool_specific_context_keys() -> None:
    ...
    assert {
        tool: context_keys_by_tool[tool] for tool in expected_subset
    } == expected_subset
