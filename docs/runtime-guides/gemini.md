...
- A **Google API key** (`GOOGLE_API_KEY`) or a valid **gcloud OAuth session** (`gcloud auth login`)
- **Python >= 3.12**
...
**Option A — Google API Key (recommended for automated workflows):**

```bash
export GOOGLE_API_KEY="your-google-api-key"
```

**Option B — OAuth (Google account, free tier):**

```bash
gcloud auth login
```

> See the [Gemini CLI authentication guide](https://github.com/google-gemini/gemini-cli#authentication) for details on obtaining a Google API key or configuring OAuth.
...
| Authentication | `GOOGLE_API_KEY` or `gcloud auth login` | OpenAI API key | Max Plan subscription |
...
```bash
echo $GOOGLE_API_KEY   # should be non-empty
```
...
- **Free tier (`gcloud auth login`)** — free within monthly quota limits; suitable for experimentation and personal projects
- **Paid API key (`GOOGLE_API_KEY`)** — standard per-token charges; see [Google AI pricing](https://ai.google.dev/pricing) for current rates
