...
|----------|--------|-------|
| macOS (ARM/Intel) | Supported | Primary development platform. Officially supported and tested. |
| Linux (x86_64/ARM64) | Untested / Community Supported | May work, but not officially tested. |
| Windows (WSL 2) | Untested / Community Supported | May work, but not officially tested. |
...
Under the hood, `GeminiCLIAdapter` talks to the local `gemini` executable by passing the prompt via the `-p`/`--prompt` flag and reading the response from stdout, making each completion a single subprocess invocation.
...
The `GeminiCLIAdapter` invokes `gemini` as a subprocess, passing the prompt via the `-p`/`--prompt` flag and reading the response from stdout. The `GeminiEventNormalizer` converts raw output (plain text or NDJSON) into the internal Ouroboros event schema, so the same workflow engine drives execution regardless of the output format the CLI emits.
