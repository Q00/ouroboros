...
The `OpenCodeRuntime` adapter launches `opencode run --format json` as a subprocess for each LLM call. The orchestrator sends the prompt as input and parses the structured JSON output.
...
Each LLM call via `opencode run` creates a visible session in OpenCode's session history. Long-running workflows with many orchestrator steps will accumulate sessions. This is a cosmetic issue — it does not affect correctness.
