...
- Registers the Ouroboros MCP/env hookup in `~/.codex/config.toml`. Native Windows Desktop MCP support is experimental: managed HTTP setup owns a local streamable-HTTP endpoint. Each serve generation creates a unique per-user, least-privilege scheduled task without force-replacing an existing task. The MCP process validates immutable generation state, acknowledges readiness with its own process identity, and exits gracefully when superseded or disabled; older tasks are retained non-destructively but exit before binding unless current. In `auto` mode, setup writes the endpoint only when `[mcp_servers.ouroboros]` is absent or setup-managed. `preserve` performs no MCP lifecycle reconciliation. `--mcp-mode http` replaces `[mcp_servers.ouroboros]` with the managed endpoint; `--mcp-mode stdio` replaces it with a command-spawned server and publishes disabled state rather than deleting operator or task artifacts. Manually clean up any pre-release `Ouroboros MCP HTTP` artifacts that are detected.
...
`~/.codex/config.toml` is not where Ouroboros stage model pins belong. Use the settings UI or the equivalent `~/.ouroboros/config.yaml` values; keep user-managed native Codex profiles when you need an explicit `--profile`. In `auto` and `preserve` modes, user-managed `[mcp_servers.ouroboros]` entries are preserved. Native Windows Desktop MCP remains experimental; use WSL 2 for native Codex CLI runtime and worker workflows.
