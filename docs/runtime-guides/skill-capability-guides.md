...
| OMP | No setup-owned capability artifact yet | Known gap: OMP (Oh My Pi, the `omp` CLI) has generic rendered capability guidance in the registry, but setup does not yet install it into a durable OMP-owned instruction artifact. Use `render_backend_skill_capability_guide("omp")` when building OMP prompts until a stable artifact surface exists. |
