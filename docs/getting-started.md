...
| Gemini CLI backend | Python >= 3.12, `npm install -g @google/gemini-cli`, `GEMINI_API_KEY` or Google OAuth (browser) |
...
# Gemini-backed flows (API key — or sign in via OAuth browser flow: run `gemini` once)
...
| **Auth** | Max Plan subscription | `OPENAI_API_KEY` | `GEMINI_API_KEY` or Google OAuth (browser) |
...
All backends run the same core workflow engine (seed execution, TUI). However, user-facing commands still differ: Claude Code has native in-session `ooo` workflows, while Codex CLI relies on `ouroboros setup --runtime codex` to install managed rules/skills plus the MCP hookup, and Gemini CLI uses `ouroboros setup --runtime gemini` to register the MCP server in `~/.gemini/settings.json`. The `ouroboros` CLI remains the most universal terminal path, and some advanced operations are still MCP/Claude-only.
