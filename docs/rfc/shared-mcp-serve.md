...
- **Project directory.** 55 reads of `Path.cwd()` or `os.getcwd()` outside
  ...
  `self._task_cwd or self._adapter.working_directory or os.getcwd()` about
  twenty times. `create_ouroboros_server` bakes the launch cwd into the default
...
- **Environment.** The project `.env` is imported into `os.environ`. The login
  shell env is merged in, and the first launcher's values win. About 20
  subprocess spawn sites derive the child env from `os.environ`.
...
- A ProjectContext is built when a session attaches and cached by its key:
  `(realpath(project_dir), runtime, llm_backend, project .env digest)`.
...
- A ProjectContext's env overlay is: trusted process env, then `~/.ouroboros/.env`
  (trusted), then the **allowlisted** keys of the project `.env`. Keys outside
  the allowlist are ignored and logged by name only. A new key is inert until
  someone adds it on purpose. The allowlist lists names explicitly, never by
  suffix or pattern.
...
Draft allowlist, from an AST scan of every `os.environ` / `getenv` read with
module constants resolved (95 keys plus 30 dynamic reads):

| Allowed from project `.env` | Keys |
|---|---|
| Model selection | `OUROBOROS_{CLARIFICATION, EXECUTION, QA, REFLECT, SEMANTIC, WONDER, VALIDATION, DETECTOR, ASSERTION_EXTRACTION, CONTEXT_COMPRESSION, DEPENDENCY_ANALYSIS, ONTOLOGY_ANALYSIS, CONSENSUS_ADVOCATE, CONSENSUS_DEVIL, CONSENSUS_JUDGE}_MODEL`, `OUROBOROS_CONSENSUS_MODELS` |
| Reasoning effort | `OUROBOROS_AGENT_REASONING_EFFORT` |
| Presentation | `OUROBOROS_LOG_MODE` |

Everything else is denied. That covers binaries, runtime selectors, config
homes, permission modes, machine-resource limits (`OUROBOROS_MAX_*`, wall-clock
and timeout keys, since a cloned repository must not be able to pin the
machine), `PATH`/`HOME`/`PYTHONPATH`, MCP/plugin roots and
telemetry endpoints.
...
This is a behavior change: today most project `.env` keys take effect. The
...
1. Reads the discovery record `~/.ouroboros/run/serve.json` (0600:
   `{protocol, version, pid, socket}`). When no live server of the **same
   package version** exists, it takes the flock `~/.ouroboros/locks/serve.lock`,
   checks again, and spawns the shared server detached, in its empty cwd, with an
   **explicit** env: trusted process basics (`PATH`, `HOME`, `TMPDIR`, locale)
   plus `~/.ouroboros/.env`. It never passes its own `os.environ`, which carries
   the login-shell merge, `_OUROBOROS_NESTED` (the spawned server would refuse
   to start) and host markers such as `CLAUDECODE`. Host identity reaches the
   server only through the attach request's `client` field.
2. Connects to the server's unix socket, which lives in a 0700 directory with a
   short path because of the macOS `sun_path` limit.
3. Authenticates (section 4). Then, in the attach request, it sends only
   `{project_dir, runtime, llm_backend, client}`. **No environment crosses the
   socket.** The server reads the project `.env` itself, through the
   allowlist.
4. Passes its own stdin/stdout descriptors over `SCM_RIGHTS`
...
When package versions differ, a new server starts beside the old one, and the
old one drains and exits.

**Fallback** to today's per-session in-process serve applies on Windows (no
`AF_UNIX` fd passing), when `OUROBOROS_SHARED_SERVE=0` (read from the real
process env only and denylisted from the project `.env`), or when attach fails
within a bounded timeout.

### 4. Authentication and authorization

- **One credential authority.** Attaching presents a credential that the
  existing `Authenticator` verifies (`hmac.compare_digest`), yielding an
  `AuthContext` (Principal). `SecurityLayer` keeps checking every tool call
...
- **Confinement.** The process-global `--workspace-root` policy
  (`set_workspace_policy`) becomes per session. The launcher sends its
  `--workspace-root` values in the attach request. The session's effective
  roots are those values intersected with the Principal's project scope, so
  one launcher's flag never widens or narrows another session.
...
5. An unauthenticated connection cannot build a ProjectContext, pass descriptors or
   call a tool.
6. The credential never appears in env, argv, logs or events.
...
|---|---|---|
| 1a | Project `.env` as allowlisted data; `build_child_env(base_env=…)` at every spawn site; no `os.environ` mutation after startup | Yes: closes the untrusted-`.env` advisory class structurally. Label `security` |
...
- Two clients in the same project: one server, one ProjectContext.
...
- Mixed package versions: separate servers.
