...
| `ac_attempt_timeout_seconds` | `int [1, 9007199254]` | `900` | Hard wall-clock ceiling for active work in one atomic provider attempt. The upper bound keeps durable microseconds exact for IEEE-754/JSON consumers. Progress messages do not extend it. Provider finalization has a separate finite one-second shutdown grace plus 100 ms cancellation observation, so cleanup cannot hang the exhausted result or renew provider authority. Whole-Seed direct/resume calls multiply the exact microsecond limit by root AC count and fail closed if the scaled value would exceed the same boundary. |
