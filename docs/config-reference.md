...
Pi is available as both an agent runtime backend and, when the Pi LLM adapter is installed, an LLM-only backend for interview, seed generation, QA, and analysis flows. The runtime uses documented JSON mode (`pi --mode json <prompt>`) and preserves Pi native session IDs for targeted resume.
