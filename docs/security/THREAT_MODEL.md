...
- Which launch paths actually reach `_ensure_shell_env` with `SHELL` unset in the real environment (GUI-launched Claude Desktop, Codex app, IDE extensions)? That determines whether the `SHELL` gap is `likely` or `possible`.
...
| Land the sibling PR adding `PYTHON*`, `NODE_PATH`, `NPM_CONFIG_*`, `YARN_*`, `PNPM_*`, `COREPACK_*`, `GIT_*` to the denylist; un-xfail the catalog cases | T1 | partial | S |
| Deny `SHELL` from the untrusted `.env` (login-shell import argv[0]) | T5,T1 | partial | S |
