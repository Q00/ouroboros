...
| 대시보드의 `r`(그리고 별도 바인딩이 없는 화면) | 실행 소유자가 연결됐을 때 재개 요청 |
...
> 바인딩보다 우선합니다. 대시보드가 의미가 분명한 생명주기 제어 화면입니다.
>
...
아래 설명은 날짜를 검증할 수 없는 트래픽·동작 스냅샷이 아니라 저장소에 들어 있는
현재 런타임 정의에 연결돼 있습니다.
