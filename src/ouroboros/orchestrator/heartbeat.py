...
_PERSISTED_PROCESS_IDENTITY_VERSION = 1
...
def current_persisted_process_identity() -> dict[str, object] | None:
    ...
    ``/proc/stat``'s epoch ``btime``, which can drift on WSL2 while a process is
    ...
    if boot_id is None or start_ticks is None:
        return None
    return {
        "version": _PERSISTED_PROCESS_IDENTITY_VERSION,
        "platform": "linux",
        "pid": pid,
        "boot_id": boot_id,
...
def persisted_process_identity_alive(
    ...
    *,
    ...
    if (
        ...
        or version != _PERSISTED_PROCESS_IDENTITY_VERSION
