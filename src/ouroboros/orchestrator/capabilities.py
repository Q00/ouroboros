...
_OUROBOROS_CANCEL_METADATA: Mapping[str, Mapping[str, Any]] = {
    ...
    "ouroboros_ralph": _OUROBOROS_UNSUPPORTED_CANCEL_METADATA,
...
def _interrupt_metadata_for_ouroboros_tool(name: str) -> Mapping[str, Any]:
    ...
    resume_companions = _filter_available_ouroboros_tools(
        metadata.get("resume_companions", ())
    )
    cancel_companions = _filter_available_ouroboros_tools(
        metadata.get("cancel_companions", ())
    )
    return {
        **metadata,
...
def _cancel_metadata_for_ouroboros_tool(name: str) -> Mapping[str, Any]:
    metadata = _OUROBOROS_CANCEL_METADATA.get(
        name, _OUROBOROS_UNSUPPORTED_CANCEL_METADATA
    )
        ...
        "allOf": [
            {
                "if": {
                    "properties": {
                        "answer_prefix": {"const": "[from-code][auto-confirmed]"}
                    },
...
def _serialized_metadata_for_descriptor(
    ...
    return {
        ...
        "interrupt": dict(descriptor.metadata.interrupt),
        "cancel": {
            key: list(value) if isinstance(value, tuple) else value
...
def validate_capability_tool_metadata(
    ...
    *,
    ...
    if not isinstance(metadata.required_context_keys, tuple) or not all(
        ...
        raise ValueError(
            f"{tool_name}: required_context_keys must be a tuple of strings"
        )
    ...
    if (
        mutation_class is not CapabilityMutationClass.READ_ONLY
        and not metadata.side_effects
    ):
        raise ValueError(
            f"{tool_name}: mutating capabilities require explicit side_effects"
        )
    if (
        mutation_class is not CapabilityMutationClass.READ_ONLY
        and not metadata.mutation_targets
    ):
        raise ValueError(
            f"{tool_name}: mutating capabilities require explicit mutation_targets"
        )
    for mutation in metadata.state_mutations:
        ...
        if not isinstance(mutation["context_keys"], tuple) or not all(
            ...
            raise ValueError(
                f"{tool_name}: state mutation context_keys must be a tuple of strings"
            )
    ...
    if not isinstance(metadata.retry["supported"], bool) or not isinstance(
        metadata.retry["mode"], str
    ...
    if metadata.interrupt.get("execution_mode") == "blocking":
        ...
        if not isinstance(metadata.interrupt["blocking_semantics"], str) or not metadata.interrupt[
            "blocking_semantics"
        ]:
            raise ValueError(
                f"{tool_name}: blocking interrupt requires blocking_semantics"
            )
        ...
        if not isinstance(metadata.interrupt["target_context_keys"], tuple):
            raise ValueError(
                f"{tool_name}: blocking interrupt target_context_keys must be a tuple"
            )
        ...
        if not isinstance(metadata.interrupt["resumable"], bool) or not isinstance(
            ...
            raise ValueError(
                f"{tool_name}: background interrupt resumable/cancellable "
                "must be booleans"
            )
        if not isinstance(metadata.interrupt["resume_companions"], tuple):
            raise ValueError(
                f"{tool_name}: background interrupt resume_companions must be a tuple"
            )
        if not isinstance(metadata.interrupt["cancel_companions"], tuple):
            raise ValueError(
                f"{tool_name}: background interrupt cancel_companions must be a tuple"
            )
        if not isinstance(metadata.interrupt["target_context_keys"], tuple):
            raise ValueError(
                f"{tool_name}: background interrupt target_context_keys must be a tuple"
            )
        if metadata.interrupt["resumable"] and not metadata.interrupt[
            "resume_companions"
        ]:
            raise ValueError(
                f"{tool_name}: resumable background interrupt requires resume companions"
            )
        if metadata.interrupt["cancellable"] and not metadata.interrupt[
            "cancel_companions"
        ]:
            raise ValueError(
        ...
        if not metadata.interrupt["target_context_keys"]:
            raise ValueError(
                f"{tool_name}: background interrupt requires target context keys"
            )
        ...
        if metadata.interrupt["mutation_semantics"] != "no_state_mutation":
            raise ValueError(
                f"{tool_name}: read-only interrupt must declare no_state_mutation"
            )
        ...
        if metadata.interrupt["target_context_keys"]:
            raise ValueError(
                f"{tool_name}: read-only interrupt must not require context keys"
            )
        ...
        if metadata.interrupt["supported"] is not True:
            raise ValueError(
                f"{tool_name}: terminal-control interrupt must be supported"
            )
        if metadata.interrupt["terminal_action"] != "cancel":
            raise ValueError(
                f"{tool_name}: terminal-control interrupt action must be cancel"
            )
        if not isinstance(metadata.interrupt["target_type"], str) or not metadata.interrupt[
            "target_type"
        ]:
            raise ValueError(
                f"{tool_name}: terminal-control interrupt requires target_type"
            )
        if not isinstance(metadata.interrupt["target_context_keys"], tuple) or not metadata.interrupt[
            "target_context_keys"
        ]:
            raise ValueError(
                f"{tool_name}: terminal-control interrupt requires target context keys"
            )
        if not isinstance(metadata.interrupt["directive_semantics"], str) or not metadata.interrupt[
            "directive_semantics"
        ]:
            raise ValueError(
                f"{tool_name}: terminal-control interrupt requires directive semantics"
            )
        if not isinstance(metadata.interrupt["terminal_statuses"], tuple) or not metadata.interrupt[
            "terminal_statuses"
        ]:
            raise ValueError(
                f"{tool_name}: terminal-control interrupt requires terminal statuses"
            )
        if not all(
            isinstance(status, str) and status
            for status in metadata.interrupt["terminal_statuses"]
        ):
            raise ValueError(
        ...
        if not isinstance(metadata.interrupt["idempotent"], bool):
            raise ValueError(
                f"{tool_name}: terminal-control interrupt idempotent must be a bool"
            )
    elif interrupt_keys != {"supported", "mode"}:
        raise ValueError(f"{tool_name}: interrupt must contain supported and mode")
    if not isinstance(metadata.interrupt["supported"], bool) or not isinstance(
        metadata.interrupt["mode"], str
    ...
    if set(metadata.cancel) != {
        ...
        raise ValueError(
            f"{tool_name}: cancel must contain supported, mode, companions, "
            "and target_context_keys"
        )
    if not isinstance(metadata.cancel["supported"], bool) or not isinstance(
        metadata.cancel["mode"], str
    ...
    if metadata.cancel["supported"] and (
        not metadata.cancel["companions"]
        or not metadata.cancel["target_context_keys"]
    ):
        raise ValueError(
            f"{tool_name}: supported cancel requires companions and context keys"
        )
    if not metadata.cancel["supported"] and (
        metadata.cancel["companions"] or metadata.cancel["target_context_keys"]
    ):
        raise ValueError(
            f"{tool_name}: unsupported cancel must carry explicit empty tuples"
        )
...
def _normalize_embedded_ouroboros_tool_capability_metadata(
    ...
    return {
        ...
        "state_mutations": [
            {
                ...
                "context_keys": list(
                    _string_tuple_from_payload(mutation.get("context_keys"))
                ),
            }
        ...
        "side_effects": list(
            _string_tuple_from_payload(raw_capability.get("side_effects"))
        ),
        "retry": dict(retry),
...
def _lateral_persona_panel_metadata() -> LateralPersonaPanelMetadata:
    ...
    return LateralPersonaPanelMetadata(
        ...
        personas=tuple(
            LateralPersonaMetadata(
                ...
                response_payload_ref={
                    ...
                    "inline_content": (
                        "content sentinel "
                        "ouroboros-lateral-inline-dispatch-v1.payloads[persona_id]"
                    ),
                },
            )
        ...
        response_payload_refs={
            ...
            "inline_content": (
                "content sentinel ouroboros-lateral-inline-dispatch-v1.payloads"
            ),
...
def _background_lifecycle_metadata_for_ouroboros_tool(name: str) -> Mapping[str, Any]:
    ...
    return {
        ...
        "runtime_instruction": (
            "After the background tool returns a job_id, use available lifecycle "
            f"companions: {', '.join(available_lifecycle_roles) or 'none'}."
        ),
    }
...
def _job_lifecycle_sibling_metadata_for_ouroboros_tool(
    ...
    current_role = next(
        role for role, tool_name in sibling_roles.items() if tool_name == name
    )
    sibling_companions = {
        role: tool_name
        for role, tool_name in sibling_roles.items()
        if tool_name != name
    }
    return {
...
def _run_family_metadata_for_ouroboros_tool(name: str) -> Mapping[str, Any]:
    ...
    companion_roles = {
        role: tool_name
        for role, tool_name in role_candidates.items()
        if tool_name in definitions
    }
    ...
    return {
        ...
        "required_result_context_keys": {
            ...
            if "execution_cancel" in companion_roles
            else (),
...
def _evaluate_family_metadata_for_ouroboros_tool(name: str) -> Mapping[str, Any]:
    ...
    return {
        ...
        "cancel": _cancel_metadata_for_ouroboros_tool(name),
        "background_cancel": _cancel_metadata_for_ouroboros_tool(
            "ouroboros_start_evaluate"
        )
...
def _ralph_family_metadata_for_ouroboros_tool(name: str) -> Mapping[str, Any]:
    ...
    return {
        ...
        "runtime_instruction": (
            "Use ouroboros_ralph for blocking Ralph loop orchestration. Use "
            "the start variant when background Ralph execution is required, "
            "then follow available Ralph family companions: "
    ...
    *,
    ...
    metadata = (
        ...
        if _is_ouroboros_owned_tool(tool)
            ...
            _generic_attached_tool_metadata(tool)
            if resolved_source.kind != "builtin"
            else None
        )
    )
                ...
                state_mutations=tuple(
                    {
                        ...
                        "context_keys": _string_tuple_from_payload(
                            value.get("context_keys")
                        ),
                    }
