...
def valid_execution_semantics_contract(value: object) -> bool:
    ...
    verify_shell_path = value.get("verify_shell_path")
    ...
    return bool(
        ...
        and timeout >= 1
        and (
            verify_shell_path is None
            or (isinstance(verify_shell_path, str) and bool(verify_shell_path))
        )
...
def migrated_pre_verify_shell_execution_semantics(
    ...
    """Migrate the exact v4 shape to conservative unavailable verification.

    Sessions created before the resolved-shell authority was durable cannot
    prove which interpreter should judge pending criteria. They resume with a
    sealed ``None`` path, preserving completed work as unverified rather than
    selecting a new live executable.
    """
    if (
        not isinstance(value, Mapping)
        or value.get("version") != PRE_VERIFY_SHELL_EXECUTION_SEMANTICS_VERSION
        or "verify_shell_path" in value
    ):
        return None
    migrated = dict(value)
    migrated["version"] = CURRENT_EXECUTION_SEMANTICS_VERSION
    migrated["verify_shell_path"] = None
...
def pre_adaptive_execution_semantics_rejection(
    ...
    *,
    ...
    candidate["verify_shell_path"] = None
