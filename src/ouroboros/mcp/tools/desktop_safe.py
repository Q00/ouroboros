    ...
    return [
        _LazyToolHandler(
            MCPToolDefinition(
                ...
                parameters=(
                    MCPToolParameter(
                        "session_id", ToolInputType.STRING, "The session ID to query", required=True
                    ),
                ),
            ),
            ...
            MCPToolDefinition(
                ...
                parameters=(
                    MCPToolParameter(
                        ...
                        "Job ID returned by a start tool",
                        required=True,
                    ),
                ),
            ),
            lambda: _load_handler(
                "JobStatusHandler", event_store=event_store, job_manager=job_manager
            ),
        ),
        _LazyToolHandler(
            MCPToolDefinition(
                ...
                parameters=(
                    ...
                    MCPToolParameter(
                        ...
                        "Previous cursor from job_status or job_wait",
                        required=False,
                        default=0,
                    ),
                    MCPToolParameter(
                        "timeout_seconds",
                        ToolInputType.INTEGER,
                        "Maximum seconds to wait for a change (longer = fewer round-trips)",
                        required=False,
                        default=30,
                    ),
                ),
            ...
            lambda: _load_handler(
                "JobWaitHandler", event_store=event_store, job_manager=job_manager
            ),
        ),
        _LazyToolHandler(
            MCPToolDefinition(
            ...
            lambda: _load_handler(
                "JobResultHandler", event_store=event_store, job_manager=job_manager
            ),
        ),
        _LazyToolHandler(
            MCPToolDefinition(
            ...
            lambda: _load_handler(
                "CancelJobHandler", event_store=event_store, job_manager=job_manager
            ),
        ),
        _LazyToolHandler(
            MCPToolDefinition(
                ...
                parameters=(
                    MCPToolParameter(
                        "session_id",
                        ToolInputType.STRING,
                        "Filter events by session ID. If not provided, returns events across all sessions.",
                        required=False,
                    ),
                    MCPToolParameter(
                        "event_type",
                        ToolInputType.STRING,
                        "Filter by event type (e.g., 'execution', 'evaluation', 'error')",
                        required=False,
                    ),
                    MCPToolParameter(
                        "limit",
            ...
            MCPToolDefinition(
                ...
                parameters=(
                    MCPToolParameter(
                        "session_id",
                        ToolInputType.STRING,
                        "The execution session ID to measure drift for",
                        required=True,
                    ),
                    MCPToolParameter(
                        "current_output",
                        ToolInputType.STRING,
                        "Current execution output to measure drift against the seed goal",
                        required=True,
                    ),
                    MCPToolParameter(
                        "seed_content",
                        ToolInputType.STRING,
                        "Original seed YAML content for drift calculation",
                        required=True,
                    ),
                    MCPToolParameter(
                    ...
                    MCPToolParameter(
                        "current_concepts",
                        ToolInputType.ARRAY,
                        "Concepts present in the current output",
                        required=False,
                    ),
                ),
            ),
            ...
            MCPToolDefinition(
                ...
                parameters=(
                    MCPToolParameter(
                        "problem_context",
                        ToolInputType.STRING,
                        "Description of the stuck situation or problem",
                        required=True,
                    ),
                    MCPToolParameter(
                        "current_approach",
                        ToolInputType.STRING,
                        "What has been tried so far that isn't working",
                        required=True,
                    ),
                    MCPToolParameter(
                        "persona",
                        ToolInputType.STRING,
                        "Specific persona to use",
                        required=False,
                    ...
                    MCPToolParameter(
                        "failed_attempts",
                        ToolInputType.ARRAY,
                        "Previous failed approaches to avoid repeating",
                        required=False,
                    ),
                ),
            ),
            ...
            MCPToolDefinition(
                ...
                parameters=(
                    MCPToolParameter(
                        "execution_id",
                        ToolInputType.STRING,
                        "The execution/session ID to cancel",
                        required=True,
                    ),
                    MCPToolParameter(
                        "reason",
