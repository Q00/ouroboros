...
**The store is asked for one thing: when.** A completed fan-out is published
...
publication time exists nowhere else -- a content-addressed store reuses a body
when the same result is published again, so the body's modified time reports
when those bytes first appeared rather than when the finding was last
established. Blob metadata answers the wrong question.

**Nothing here reads storage directly.** Publication times come from the store's
own query and bodies from its ``fetch``, which is bounded, contained and
verified against its address. That is not a boundary against a project -- RFC
#2153 puts the project workspace inside the trust boundary, and anything able to
write there can more easily edit the source a lane would cite. It is ownership:
this module knew the on-disk shape of a contract record, and a store that stops
being a directory would have broken it silently.
...
def _eligible_lane_outputs(body: Any) -> list[tuple[str, Any]]:
    ...
    Empty for a body of another fan-out kind, and empty for a body whose lanes
    are all ineligible -- an interview turn runs six lanes, and a contrarian's
    challenge reused as evidence answers a question nobody asked.
    """
...
def recent_findings_entries(
    ...
    *,
    ...
    entries: list[dict] = []
    for published in findings_store.published_contracts(
