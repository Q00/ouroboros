"""OMP (Oh My Pi) CLI adapter for LLM completion via ``omp --mode json``."""
...
class OmpLLMAdapter(CodexCliLLMAdapter):
    """LLM adapter backed by ``omp --mode json``.

    OMP is a Pi-family coding agent and emits the same JSONL event stream as
    the Pi adapter, exposed here as an LLM-only provider so
    interview/planning/evaluation roles can select ``--llm-backend omp``.
    """

    _provider_name = "omp"
    _display_name = "OMP CLI"
    _default_cli_name = "omp"
    _tempfile_prefix = "ouroboros-omp-llm-"
    _schema_tempfile_prefix = "ouroboros-omp-schema-"
    _log_namespace = "omp_llm_adapter"
    _completion_profile_backend = "omp"
    ...
    def _build_permission_args(self) -> list[str]:
        """OMP JSON mode does not currently expose Codex-style permission flags."""
        return []
    ...
    def _codex_failure_details(
        ...
        *,
        ...
        """Keep OMP failures backend-neutral despite the shared subprocess base."""
    ...
    def _prompt_stdin_bytes(self, prompt: str) -> bytes | None:
        """OMP JSON mode receives the prompt as a positional argument.

        Verified against omp v18: piping the prompt on stdin *in addition* to
        the positional argument duplicates it in the user message, so stdin
        stays unused.
        """
        del prompt
        return None
    ...
    def _build_command(
        ...
        *,
        ...
        """Build ``omp --mode json <prompt>``.

        ``output_last_message_path`` and schema/profile parameters are accepted
        for factory compatibility with :class:`CodexCliLLMAdapter`; OMP emits
        its response on JSONL stdout instead.
        """
        ...
        # sentinel "default". OMP's ``--model`` does fuzzy matching and has no
        # "default" id, so the sentinel is omitted and OMP uses its own
        # configured model.
    ...
    def _build_response_format_directive(
        ...
        """Translate response_format into OMP prompt instructions.

        OMP JSON mode does not expose a hard ``--output-schema`` flag, so
        structured output is cooperatively enforced through the prompt and
        validated after extraction (same contract as the Pi adapter).
        """
    ...
    def _update_last_content(self, last_content: str, event_content: str) -> str:
        """Accumulate streaming deltas but replace them with terminal OMP content.

        OMP JSON mode emits both whitespace-preserving ``message_update``
        deltas and a final ``agent_end`` / ``message_end`` full message. The
        inherited Codex loop only passes extracted text into this hook, so
        this adapter records the most recent OMP event kind in
        ``_extract_text`` and uses it here to avoid returning duplicated
        ``delta + final`` content.
        """
        if not event_content:
            return last_content
        if self._last_omp_event_kind == "final":
    ...
    async def complete(
        ...
        """Make an OMP completion request, including soft structured output support."""
        self._last_omp_event_kind = None
        ...
        if not directive:
            return Result.err(
                ProviderError(
                    message="Unsupported OMP structured response_format request",
        ...
        return Result.err(
            ProviderError(
                message="JSON format required but OMP returned non-conforming output",
                provider=self._provider_name,
    ...
    def _extract_text_from_message(self, message: dict[str, Any]) -> str:
        """Extract assistant text from an OMP transcript message."""
    ...
    def _extract_content_delta(self, event: dict[str, Any]) -> str:
        """Extract streaming text with parity to ``OmpRuntime`` JSON parsing."""
    ...
    def _extract_error_content(self, event: dict[str, Any]) -> str | None:
        """Extract OMP assistant error text from zero-exit JSON events.

        OMP can report provider/auth failures as assistant messages with
        ``stopReason: "error"`` while the CLI process still exits 0 (same
        family as Pi). Treat those as provider errors so LLM callers see the
        actionable OMP message instead of a generic empty response.
        """
    ...
    def _extract_final_text(self, event: dict[str, Any]) -> str:
        """Extract only terminal assistant content from OMP final events."""
        event_type = event.get("type")
    ...
    def _extract_text(self, value: object) -> str:
        """Extract content from documented OMP JSONL events."""
        if isinstance(value, dict):
            event_type = value.get("type")
            self._last_omp_event_kind = None
            if event_type == "message_update":
                self._last_omp_event_kind = "delta"
            ...
            if event_type in {"message_start", "message_end", "turn_end", "agent_end"}:
                ...
                if event_type == "message_start":
                    return ""
                self._last_omp_event_kind = "final"
                return self._extract_final_text(value)
            # OMP control/metadata events (for example `session`) must never
            # fall through to the broad Codex extractor, which treats shallow
            # string fields such as `type` and `id` as user-visible text.
            return ""
        self._last_omp_event_kind = None
        return super()._extract_text(value)


__all__ = ["OmpLLMAdapter"]
