...
class CodexCliLLMAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_mode = resolve_codex_permission_mode(permission_mode, default_mode="default")
    ...
    async def _complete_once(
        ...
        output_fd, output_path_str = tempfile.mkstemp(prefix="ouroboros-codex-llm-", suffix=".txt")
        os.close(output_fd)
        ...
        try:
            ...
            return Result.err(
                ProviderError(
                    message=f"Codex CLI not found: {exc}",
                    provider="codex_cli",
        ...
        if not content:
            content = self._fallback_content(stdout_lines, stderr)

        if process.returncode != 0:
            return Result.err(
                ProviderError(
                    message=content or f"Codex CLI exited with code {process.returncode}",
                    provider="codex_cli",
                    details={
        ...
        if not content:
            return Result.err(
                ProviderError(
                    message="Empty response from Codex CLI",
                    provider="codex_cli",
    ...
    async def complete(
        ...
        for attempt in range(self._max_retries):
            ...
            if not self._is_retryable_error(result.error.message) or attempt >= self._max_retries - 1:
                return result

            await asyncio.sleep(2**attempt)

        return Result.err(last_error or ProviderError("Codex CLI request failed", provider="codex_cli"))
