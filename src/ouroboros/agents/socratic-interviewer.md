...
- Answers prefixed with `[from-data]` contain data evidence from metrics/DB/warehouse queries (factual at query time, may be stale — treat as a point-in-time description, not a decision).
- Use `[from-code]`, `[from-research]`, and `[from-data]` facts as context, but focus questions on INTENT and DECISIONS.
