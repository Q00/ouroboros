...
async def _list_stdio_mcp_tool_names(
    ...
    """Launch Codex's configured stdio MCP command and return its tool names.

    Delegates to the dependency-free probe in
    :mod:`ouroboros.cli.stdio_mcp_probe`: Codex can point at a self-contained
    command such as
    ``uvx --isolated --python >=3.12 --from ouroboros-ai[mcp] ouroboros mcp serve``,
    and validating that setup must not first require the current
    ``ouroboros codex doctor`` interpreter to have installed the optional
    local ``mcp`` extra.
    """
    from ouroboros.cli.stdio_mcp_probe import list_stdio_mcp_tool_names

    return await list_stdio_mcp_tool_names(command, args, env)
