...
- **Claude Code**: Tools are deferred — use `ToolSearch` to load them:
  ...
  ```
- **Cursor / other MCP clients**: Tools are auto-loaded when the server connects. They should already be callable as `ouroboros_lateral_think`.

If the tool is available → proceed to **MCP mode below**. If not → skip to **Fallback** section.
