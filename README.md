...
| `ooo evaluate` | *(skill)* | 3-stage verification gate |
| `ooo evolve` | *(skill)* | Evolutionary loop until ontology converges |
| `ooo unstuck` | *(skill)* | 5 lateral thinking personas when you are stuck |
| `ooo status` | `ouroboros status executions` / `ouroboros status execution <id>` | Session tracking + drift detection |
| `ooo cancel` | `ouroboros cancel execution [<id>\|--all]` | Cancel stuck or orphaned executions |
| `ooo ralph` | *(skill)* | Persistent loop until verified |
| `ooo tutorial` | *(interactive)* | Interactive hands-on learning |
| `ooo help` | `ouroboros --help` | Full reference |
| `ooo pm` | *(skill)* | PM-focused interview + PRD generation |
| `ooo qa` | *(skill)* | General-purpose QA verdict for any artifact |
...
> All `ooo` commands work as native skills in Claude Code and Cursor. Some (`evaluate`, `evolve`, `unstuck`, `ralph`) don't have standalone CLI equivalents.
