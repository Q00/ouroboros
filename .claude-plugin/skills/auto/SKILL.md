...
at most 60 seconds until the observer returns its terminal summary. Child
`send_message` calls only queue mailbox events and cannot revive an ended parent
turn. Relay meaningful updates, handle user input if it interrupts the wait,
and resume waiting while the observer is active. This relay loop must not poll
the job or take cursor ownership. If child creation fails, do not promise live
proactive relays. The detached worker survives the stdio turn; catch up from
durable events on the next parent turn or explicit status request. Keep the
fallback polling loop open only for explicit live watching.
