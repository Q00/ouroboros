...
Ouroboros adaptation: the upstream canary fixture is not vendored. Smoke
test against this repository's own scanner output:

```
/triage docs/security/VULN-FINDINGS.json --auto --repo . --fp-rules .claude/fp-rules.txt
```
...
- **Checkpoints are per-phase JSON**, not conversation state. A session
  resume restores transcript history but
