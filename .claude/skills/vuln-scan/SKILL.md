...
For execution-verified findings a human builds a PoC out of band; no
execution pipeline is vendored in this repository (Ouroboros adaptation).
...
1. Resolve `<target-dir>`. If it doesn't exist or has no source files, stop
   with an error.
2. Look for `<target-dir>/THREAT_MODEL.md`, then `docs/security/THREAT_MODEL.md`
   (Ouroboros adaptation). If present, parse its section 3 "Entry
...
Write **both** files to `<target-dir>/` (Ouroboros adaptation: when the
target is `src/ouroboros`, write them to `docs/security/` instead so scanner
output never lands inside the shipped package):
...
4. Remind: these are **static candidates**, not verified. `/triage` is the
   verification step in this repository (Ouroboros adaptation).

## Constraints

- **Never execute target code.** No Bash, no builds, no `docker`, no network.
  If the user asks you to "reproduce" or "confirm with a PoC," decline;
  a human builds PoCs out of band (Ouroboros adaptation).
