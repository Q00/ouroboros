...
`/vuln-scan` focus areas and to inform how you prioritize `/triage`
results (Ouroboros adaptation: no autonomous pipeline is vendored).
...
pipeline with. If you want the skill itself to request a model, add a
`model:` line to `SKILL.md` frontmatter. It overrides the session model
(however set, `/model` or `--model`) only for the turn the skill is invoked
...
```
/threat-model bootstrap src/ouroboros
/threat-model bootstrap src/ouroboros --vulns docs/security/vulns.txt
```
...
`docs/security/THREAT_MODEL.md`. On small targets (<50 source files) it runs
...
```
/threat-model interview src/ouroboros
/threat-model interview src/ouroboros --design-doc docs/architecture.md
```
...
```
/threat-model bootstrap-then-interview src/ouroboros
```
...
```
/threat-model interview src/ouroboros --seed docs/security/THREAT_MODEL.md
```
...
example lives at `docs/security/THREAT_MODEL.md` (Ouroboros adaptation:
the canonical output for this repository is kept there, not next to the
scanned package).

## Notes

- The skill is read-only — it does not build, run, or probe the target —
...
- This repo's `docs/security/README.md` (Ouroboros adaptation).
