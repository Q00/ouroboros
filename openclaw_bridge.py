...
Usage:
    ...
    python openclaw_bridge.py start "I want to build a task management CLI"

    # Ask next question (reads state from session file)
    python openclaw_bridge.py ask <session-id>

    # Record user response
    python openclaw_bridge.py respond <session-id> "The tool should work offline"

    # Score ambiguity
    python openclaw_bridge.py score <session-id>

    # Generate seed spec
    python openclaw_bridge.py seed <session-id>

    # Show current state
    python openclaw_bridge.py status <session-id>
"""
...
from ouroboros.bigbang.ambiguity import (  # noqa: E402
    AMBIGUITY_THRESHOLD,
    AmbiguityScorer,
)
from ouroboros.bigbang.interview import (  # noqa: E402
    InterviewEngine,
    InterviewRound,
    InterviewState,
    InterviewStatus,
)
from ouroboros.bigbang.seed_generator import SeedGenerator  # noqa: E402
from ouroboros.providers.litellm_adapter import LiteLLMAdapter  # noqa: E402

# --- Config ---
SESSIONS_DIR = Path(
    os.environ.get(
        "SOCRATIC_SESSIONS_DIR", Path.home() / "openclaw" / "workspace" / "socratic-sessions"
    )
)
...
async def cmd_start(initial_context: str) -> None:
    ...
    # Record the question as a round
...
def main():
    ...
    # start
    p_start = subparsers.add_parser("start", help="Start new interview")
    p_start.add_argument("context", help="Initial project context/idea")
    ...
    # score
    p_score = subparsers.add_parser("score", help="Score ambiguity")
    p_score.add_argument("session_id", help="Session ID")

    # seed
    p_seed = subparsers.add_parser("seed", help="Generate seed spec")
    p_seed.add_argument("session_id", help="Session ID")

    # status
    p_status = subparsers.add_parser("status", help="Show interview state")
    p_status.add_argument("session_id", help="Session ID")

    # complete
    p_complete = subparsers.add_parser("complete", help="Mark interview as done")
    p_complete.add_argument("session_id", help="Session ID")
