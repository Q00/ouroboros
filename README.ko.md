...
**임계값: Ambiguity ≤ 0.2.** 인터뷰가 열려 있는 동안에는 이 값을 넘으면 Seed 생성이 막힙니다. 넘는 길은 둘입니다. CLI가 화면에 띄워주는 `force`를 명시적으로 넘기거나, 인터뷰를 완료로 표시하는 것인데 후자는 우회처럼 보이지도 않습니다. 이 게이트는 잠금장치가 아니라 반박할 수 있는 기본값입니다.
