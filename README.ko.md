...
> Claude Code의 `/status`, `/help`, `/config`는 예약 명령입니다. Ouroboros
> skill을 직접 호출할 때는 `/ouroboros:ouroboros-status`,
> `/ouroboros:ouroboros-help`, `/ouroboros:ouroboros-config`를 사용하세요.
> 기존 `ooo status`, `ooo help`, `ooo config` 문구도 계속 지원됩니다.
