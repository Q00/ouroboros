...
| **规约校验**   | 没有规约 —— 写到一半架构开始飘   | 不可变的 seed 规约锁住意图；访谈开着时，模糊度门槛（≤ 0.2）会拦下提前进入 code 的尝试       |
...
**阈值：Ambiguity ≤ 0.2。** 访谈还开着的时候，高于这个值会挡住 Seed 生成。绕过它的路有两条：显式传入 CLI 在屏幕上给出的 `force`，或者把访谈标记为完成——后者看起来根本不像在绕过。这道门槛是一个可以被反驳的默认值，不是锁。
