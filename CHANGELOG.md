...
- **evidence/verifier**: Admit a named `files_touched` claim on a verification-only run. A dependent AC that finds its files already built by a sibling, runs the suite, and lists those files as touched was rejected as `FABRICATION_SUSPECTED` (bench run `orch_777fb6248525`: 4 tests passed, AC failed twice, run ended 3/4). When the harness's own snapshot diff witnessed zero mutation, the verify gate is active, and the named path is a real workspace file, the claim is a mislabelled verification and no longer fails the AC. Ghost paths, any observed mutation, a truncated snapshot, or a missing witness still reject.
