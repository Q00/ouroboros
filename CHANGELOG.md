...
### Security
- **run/check-package**: The check package boundary of `ooo run` executes model-written Python check scripts (on throwaway copies of the project, before and after the worker). Scripts now run with only an allowlist of environment variables (no credential-like variables), the project's virtualenv interpreter when one exists (else `python3`), and the existing per-check timeout. Residual risk: there is no OS sandbox, so a check can read files the user can read and use the network. Opt out with `--no-check-package`, `OUROBOROS_CHECK_PACKAGE=off`, or `boundary.check_package: off`. A project `.env` can no longer set `OUROBOROS_CHECK_PACKAGE`.
- **config/untrusted-env**: Deny runtime module-resolution controls from the untrusted project `.env` (incomplete-fix follow-up to CVE-2026-66065, GHSA-7j6g-gw2r-mw48). `PYTHONPATH`, `PYTHONHOME`, `PYTHONSTARTUP`, `PYTHONUSERBASE`, `PYTHONEXECUTABLE` and `NODE_PATH` were absent from the denylist, so a cloned repository's `.env` could set `PYTHONPATH` and every child spawned from `os.environ` — the detached job worker, the dashboard daemon, vendor CLIs — imported an attacker `sitecustomize` before any Ouroboros code ran (arbitrary code execution as the user). The `NPM_CONFIG_`/`YARN_`/`PNPM_`/`COREPACK_` families (npm reads every `npm_config_*` as a config key, including `script-shell`) and the `GIT_` family (`GIT_SSH_COMMAND`, `GIT_EXEC_PATH`, `GIT_CONFIG_GLOBAL`, `GIT_ASKPASS`, … each name a program git then runs) are denied for the same reason. Same-class keys found by the accompanying threat-model scan are denied in the same change: `CLAUDE_CONFIG_DIR` (Claude Code settings root, like `CODEX_HOME`), `ZDOTDIR` and `SHELL` (login-shell environment import), `BROWSER` (`webbrowser.open` executes it), `OUROBOROS_BACKEND_LIMITS` (config root), `OUROBOROS_MCP_AUTH_TOKEN` (network MCP secret), `OUROBOROS_IO_JOURNAL_PREVIEWS` (privacy switch). Trusted sources (real process environment, `~/.ouroboros/.env`) are unaffected. Reported by 0xRomSec.

### Changed
- **telemetry**: Notice v4. `workflow_outcome` for `command=run` gains closed-enum check-package dimensions (`check_package_arm`, `check_package_assignment`, `check_package_status`, `package_verdict`, `legacy_verdict`, `reconciliation`, `legacy_failure_class`, `legacy_failure_class_count`). The first-run notice names randomized defaults and research use and is re-displayed once to existing installs through a new `notice_version` field in `telemetry.json` (Python and `install.sh`).
...
### Fixed
- **telemetry/cli**: CLI `ooo run` outcomes were recorded as `command=extension_job` because the CLI's job type `run` was not a canonical job type; they are now `command=run`.
- **evidence/verifier**: Anchor an inline `python -c` verification to the workspace modules it imports, so a correct fix verified with `python3 -c "from mod import f; assert ..."` is no longer rejected as an evidence-form mismatch. The tier stays fail-closed (a real workspace file, a recorded command, and an authoritative zero exit are still required).
- **telemetry/cli**: Count non-success `ooo run` outcomes. The CLI `workflow_outcome` added for #2311 was queued only on a runner result and then raised `typer.Exit` before the daemon sender posted it, and it emitted nothing at all for an orchestrator-level error, so fleet success rate counted CLI successes but dropped CLI failures. The outcome is now recorded for every terminal result (runner failure, cancellation, orchestrator error), carries the same closed `failure_cause` an MCP job derives from durable executor evidence, gets a fresh id per invocation so a `--resume` after a failure is its own outcome, and the command waits (bounded, 1.5 s) for the sender before exiting. A paused run emits nothing; its resume does.
- **evidence/verifier**: Recognise a command the leaf ran through a nested shell wrapper. codex sometimes records `/bin/zsh -lc "/bin/zsh -lc '<cmd>'"`; the normaliser peeled one layer, so the exact command the leaf ran was rejected as an unsupported `commands_run`/`tests_passed` claim (bench `orch_89ece88c9d11`). Wrapper layers are now peeled until the body is no longer a shell `-c` form (bounded), for both claim aliases and test-invocation extraction. A different inner command still does not alias.
- **evidence/verifier**: Admit a named `files_touched` claim on a verification-only run. A dependent AC that finds its files already built by a sibling, runs the suite, and lists those files as touched was rejected as `FABRICATION_SUSPECTED` (bench run `orch_777fb6248525`: 4 tests passed, AC failed twice, run ended 3/4). When the AC carries a success contract (its hidden verify gate stays the behavioural authority), the harness's own snapshot diff witnessed zero mutation, and the named path is a real workspace file, the claim is a mislabelled verification and no longer fails the AC. Prose ACs, ghost paths, any observed mutation, a truncated snapshot, or a missing witness still reject.
- **evidence/verifier**: Stop rejecting transcript-proven functional checks as fabrication. The `tests_passed` functional tier required the invoked artifact to be this run's own mutation and the AC to carry a success contract; a dependent AC that edits only its test file and then runs the sibling-built artifact (the canonical `python3 habit_tracker.py unknown-command; test $? -eq 2` check) was rejected as `FABRICATION_SUSPECTED` on every attempt — 43 of 90 such rejections on the 2026-09 bench had the exact command in the transcript with exit 0. The tier now requires the invoked file to be a real workspace file at verification time instead of this run's mutation, and neither the tier nor the empty-`files_touched` tolerance requires a success contract any more. Structured command match, integer zero exit, and the ghost-path rejection are unchanged.
- **mcp/security**: Stop rejecting the interview-less seed path's own input. `ouroboros_generate_seed(session_context=...)` failed with `Shell metacharacter detected` whenever an acceptance criterion was a shell chain (`ruff check && pytest`) or a decision contained a semicolon, while the identical text passed as `seed_content`. `session_context` is now a freetext container: every leaf is verbatim Seed prose and none reaches a shell. Sibling arguments such as `session_id` are still scanned. The `ooo seed` skill now documents the interview-less path and its gap-question loop.
- **gjc/backend**: Replace the removed `gjc --mode rpc` transport with GJC's supported Coordinator MCP and Broker-managed SDK lifecycle. `runtime_backend: gjc` and `llm.backend: gjc` now create durable SDK sessions, preserve session/question correlation through runtime handles, recover final assistant output, and keep existing soft structured-output validation without reading private GJC endpoint credentials (#2301).
...
### Added
- **run/check-package**: Add the check package boundary as a randomized default for `ooo run` (CLI and `ouroboros_execute_seed`). Each installation is assigned `on` or `off` (50/50) from its anonymous telemetry ID; without telemetry, or before the updated telemetry notice was shown, the arm is `off`. `--check-package/--no-check-package`, `OUROBOROS_CHECK_PACKAGE`, and `boundary.check_package` always win. With the arm `on`, the admitted package decides the criteria it covers inside the runner, before the terminal status is recorded, so the session status (`ooo status`, MCP status tools), the exit code, and telemetry all show the reconciled outcome; the existing verifier's verdict is kept per criterion as advisory. Criteria that can only be checked by reading prose are left to the existing verifier: admission rejects checks that only match text in documentation files. The package records the constructor's resolved model instead of `default`. The package freezes per-criterion oracles (expected behavior as data, including held-out cases absent from the Seed text) that the product harness calls through a binding; after the worker stops, an entry point the worker declares in its evidence (`entry_points`, optional) is validated and used where the default binding does not resolve. Each check is tier `A` (default binding), `A'` (validated declared binding), `U` (unverified), or `C` (rejected at admission: dynamic import or exec of workspace files, exec/eval of workspace content, network use, or prose matching). With the arm `on` the legacy verifier only annotates: retries follow check package counterexamples (when only held-out cases fail, one of them is revealed and retired), and a run with no failed or indeterminate criterion exits 0 and lists its unverified criteria. When no package is admitted (construction failed, rejected, or timed out with no completed check), the run falls back to the legacy verifier exactly as in arm `off`. `workflow_outcome` adds `unverified_count` and `check_tier_summary`.
- **telemetry**: `command_run` rows for `command=seed` carry one closed-enum `origin` (`interview` / `session_context` / `session_context_gap`) so adoption of the interview-less Seed path, and whether it closes into a Seed or bounces back gap questions, is visible without any Seed text leaving the machine. Documented in TELEMETRY.md; every other command and any other value drops the property.
...
- **setup/installer**: `ouroboros setup --runtime omp` wires the managed OMP bridge
  ...
  and model-catalog discovery) resolves the executable through one validated
  `OUROBOROS_OMP_CLI_PATH` > `orchestrator.omp_cli_path` > `PATH` resolver owned by
  `config/_omp_cli.py`, which validates each source independently so a stale
  higher-priority candidate falls through to the next source;
