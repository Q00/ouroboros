...
### Added
- **project map**: Add the Project Map V1 identity anchor for runner-owned sessions. One canonical resolver joins nested checkouts and positively owned linked/managed worktrees to a stable identity-root UUIDv5 while retaining a relative workspace scope; bare-owned peers, including repositories literally named `.git` and bounded contained symlink-`HEAD` representations, share their common Git directory, while unowned direct external gitfiles and aliases fail closed. Complete bounded Git metadata preserves relevant case-insensitive section, complete quoted-subsection, single leading BOM, comment, ASCII valueless/numeric boolean, quoting, escape, same-line and ordinary variable-continuation, worktree-specific config, literal Git-directory-relative `core.worktree`, and directory/gitfile explicit-owner precedence semantics while rejecting environment expansion, malformed managed scope, multiline section headers, normalization drift, malformed `.git` entries, uncontained symlinks, trailing, or oversized data; one resolved identity or resolved absence is atomically shared by the immutable session-start event and existing execution contract without gaining execution or Final Gate authority.
