...
### Added
- **project map**: Add the Project Map V1 identity anchor for runner-owned sessions. One canonical resolver joins nested checkouts and provable linked/managed worktrees—including external-gitdir and bare-owned worktree topologies—to a stable identity-root UUIDv5 while retaining a relative workspace scope; complete bounded Git pointer records fail closed on whitespace normalization, symlinks, trailing, or oversized data, and the immutable session-start event and existing execution contract share that exact identity without gaining execution or Final Gate authority.
