...
### Fixed
- **skills/claude**: Stop shadowing Claude Code's reserved `/status`, `/help`,
  and `/config` built-ins. The Claude-facing skill names are now the explicit
  `/ouroboros:ouroboros-status`, `/ouroboros:ouroboros-help`, and
  `/ouroboros:ouroboros-config`; `ooo status`, `ooo help`, `ooo config`, and
  the non-reserved `drift` alias remain available.
