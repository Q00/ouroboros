...
### Changed
- **gjc/integration**: Replace the GJC input-interception bridge with an OMP-style runtime projection when the installed host exposes conventional MCP autoload. `ouroboros setup --runtime gjc` installs namespaced `ouroboros-*` skills, an exact routing guide, and an isolated native MCP registration on capable GJC releases; older storage-only hosts retain a setup-owned compatibility bridge so `ooo` remains usable. GJC probing and registration live in the adapter, setup only orders independent publications, and the runtime config is committed after activation; failed setup preserves complete setup-owned generations for refresh instead of running a cross-artifact rollback transaction.
