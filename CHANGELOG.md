...
### Changed
- **interview/research**: Replace blanket per-question six-lane advisory fan-out with one bounded factual snapshot (`code_context` + `web_context`) published through the project artifact store. Complete-cache follow-ups emit zero advisory subagents; partial, stale, malformed, or missing baselines rerun only the missing lane, and successful answer/resume repairs become reusable. Web references now require parent-runtime search-result and successful-fetch evidence before publication. Measurements run directly only when a question needs them, while milestone lateral review and the seed-ready acceptance guard remain fresh independent checks.
