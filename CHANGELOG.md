...
### Changed
- **evidence/verifier**: Corroborate `tests_passed` and `commands_run` claims by replaying the commands the worker actually ran instead of recognizing test runners. Correct work verified with `make test`, `npm test`, `./run_tests.sh`, a piped `pytest -q | tail -5`, or claimed in free text such as `migrations (578 tests)` was rejected as `FABRICATION_SUSPECTED` or `EVIDENCE_FORM_MISMATCH`. When the transcript cannot prove a claim, the harness takes linked commands from the transcript's Bash calls (never from claim text), at most 3 per criterion, and replays each one as a direct argv in a fresh copy of the workspace with the scrubbed verify environment and timeout, network denied where the platform allows (`sandbox-exec` on macOS, a network namespace on Linux; otherwise recorded as not isolated). A claim is corroborated only by exit 0 with no timeout and no change to a pre-existing file; it is linked when it equals or contains the command, or names a test target that is an argument of it. `CMD | tail/head/grep/sed/...` replays `CMD` and uses its exit code; other shell constructs are not replayed; privilege, network, container, deletion, version-control write and package-install commands are never replayed and keep the transcript-only rules. Behavior change: re-execution no longer runs in the live workspace and no longer runs commands taken from the worker's claims. Applies only while `execution.run_verify_commands` is on. The worker's evidence request now asks for exact executed command strings, one per item, without annotations; older free-text claims still link through the rules above. See `docs/contributing/verifier-evidence-policy.md`.
