...
- **orchestrator/routing**: Propagate recoverable quota pauses across an active parallel batch immediately. A shared signal closes semaphore-waiting provider entrances, cancels running siblings, defers decomposed legacy recovery, and keeps interrupted ACs pending without failure judgments, completed-stage checkpoints, or successor effects. Direct resumed successors now build prompts from the persisted strategy rather than the mutable task-type registry.
