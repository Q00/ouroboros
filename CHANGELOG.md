...
### Added
- **project map**: Add the Project Map V1 identity anchor for runner-owned sessions. One Git-backed resolver joins nested checkouts and positively owned linked/managed worktrees to a stable identity-root UUIDv5 while retaining a relative workspace scope. The installed Git binary owns config, gitfile, common-directory, worktree, and `HEAD` semantics; bounded non-interactive queries require the active checkout to appear in Git's worktree population or match Git's explicit `core.worktree` top level, so redirected unowned markers fail closed. The existing untrusted `.env` boundary denies dynamic-loader injection before Git starts. One resolved identity is atomically shared by the immutable session-start event and existing execution contract, while omitted runtime cwd values are captured consistently across publication, spawn, and persistent resume, without granting execution or Final Gate authority.
