...
- **orchestrator/resume**: Version the durable execution contract to v7 and fingerprint every resolved runner/executor setting that affects provider prompts, effects, or acceptance. The exact context-pack fragment, complete execution profile, inherited parent runtime handle, prompt strategy, and tool catalog are sealed before provider entry. Resume rejects incomplete legacy contracts plus verification, retry, decomposition, requested/effective worker, backend concurrency/rate, harness, replay, checkpoint, or signal-capability drift before analyzer/executor/provider entry, and execution consumes the immutable snapshot instead of rereading live workspace/config inputs.
