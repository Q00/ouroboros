...
### Changed
- **orchestrator/worktrees**: Bound managed auto-worktree disk growth by default. `orchestrator.worktree_cleanup` now defaults to `remove` instead of `keep`, and a new `orchestrator.worktree_retention_hours` (default `24`, `0` disables) lets the next auto session reclaim worktrees abandoned by sessions that crashed before their release path ran — the case the completion-time policy alone could never bound. Neither path touches a dirty checkout or force-deletes a branch, so unmerged commits survive on their `ooo/*` branch. Measured on a real install: 2,586 `auto_*` worktrees / 85 GB accumulated under `~/.ouroboros/worktrees` because nothing removed them without a manual `ouroboros cleanup`.
...
### Fixed
- **auto/worktrees**: Reprovision the managed worktree on resume when the previous one was reclaimed. `state.cwd` points into the prior worktree once a session has run, so a cleanup policy that removes it left `state.cwd` dangling and `ensure_auto_worktree` silently returned `None` (running the session in the caller's checkout). Resume now falls back to the workspace's recorded repo root.
