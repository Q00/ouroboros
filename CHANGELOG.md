...
### Added
- **project map**: Add the Project Map V1 identity anchor for runner-owned sessions. One canonical resolver joins nested checkouts and provable linked/managed worktrees—including repositories with external common Git directories—to a stable identity-root UUIDv5 while retaining a relative workspace scope; the immutable session-start event and existing execution contract now share that exact identity without gaining execution or Final Gate authority.
